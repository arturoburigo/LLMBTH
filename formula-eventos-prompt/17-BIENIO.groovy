...
if (vvar < 0 && vvarFgts < 0) {
    qntAnosAdicional = AdicionaisTempoServico.busca(TipoAdicional.QUANTIDADE_ADICIONAIS, ClassificacaoAdicional.BIENIO)
    buscouNoRH = true
} else {
    if (vvar < 0) {
        somenteFgts = true
    ...
    qntAnosAdicional = Datas.diferencaAnos(dtbase, dfin) / 2
...
if (vvar > 0) {
    ...
    if (vvarFgts > 0) {
        ...
        if (buscouNoRH) {
            valorReferencia = AdicionaisTempoServico.busca(TipoAdicional.PERCENTUAL_TEMPO_SERVICO, ClassificacaoAdicional.BIENIO)
        } else {
