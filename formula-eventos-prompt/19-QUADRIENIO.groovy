...
if (vvar < 0 && vvarFgts < 0) {
    qntAnosAdicional = AdicionaisTempoServico.busca(TipoAdicional.QUANTIDADE_ADICIONAIS, ClassificacaoAdicional.QUADRIENIO)
    buscouNoRH = true
} else {
    if (vvar < 0) {
        somenteFgts = true
    ...
    qntAnosAdicional = Datas.diferencaAnos(dtbase, dfin) / 4
