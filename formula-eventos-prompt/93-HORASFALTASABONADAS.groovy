...
if (vaux <= 0) {
    vaux = Funcoes.buscaAbonos(TipoAbono.ABONADO, calculo.competencia, TipoFalta.NAO_JUSTIFICADA)
    if (vaux <= 0) {
        suspender "Não foram encontradas faltas à abonar para a matrícula na competência com os dados buscados"
    }
}
