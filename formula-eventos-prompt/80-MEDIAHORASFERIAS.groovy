...
if (TipoProcessamento.FERIAS.equals(calculo.tipoProcessamento)) {
    ...
    if (folha.folhaPagamento) {
        ...
        if (vaux > 0) {
            ...
            def base = Eventos.valor(81) + Eventos.valor(82) + Eventos.valor(234) //apenas para gerar dependência
            def vprop = mediaVantagem.calcular(periodoAquisitivo)
            if (vprop <= 0) {
                suspender "Não há média horas de férias a serem pagas ao funcionário"
            }
