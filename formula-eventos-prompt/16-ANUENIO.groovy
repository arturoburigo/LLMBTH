...
if (vvar < 0 && vvarFgts < 0) {
    qntAnosAdicional = AdicionaisTempoServico.busca(TipoAdicional.QUANTIDADE_ADICIONAIS, ClassificacaoAdicional.ANUENIO)
    buscouNoRH = true
} else {
    if (vvar < 0) {
        somenteFgts = true
    ...
    qntAnosAdicional = Datas.diferencaAnos(dtbase, dfin)
...
if (vvar > 0) {
    ...
    if (vvarFgts > 0) {
        ...
        if (buscouNoRH) {
            valorReferencia = AdicionaisTempoServico.busca(TipoAdicional.PERCENTUAL_TEMPO_SERVICO, ClassificacaoAdicional.ANUENIO)
        } else {
